...
function widthPercentageToDP(widthPercentage) {
  const element = typeof widthPercentage === 'number' ? widthPercentage : parseFloat(widthPercentage);
  return PixelRatio.roundToNearestPixel((SCREENWIDTH * element) / 100);
}

export function heightPercentageToDP(heightPercentage) {
  const element = typeof heightPercent === 'number' ? heightPercentage : parseFloat(heightPercentage);
